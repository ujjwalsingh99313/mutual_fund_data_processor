...
def process_aum_dataframe(df, source_file="manual_upload.xlsx", table_name="cam_aum_wbr22"):
    """
    Process CAM AUM DataFrame, clean and insert/update into table.
    - Skip inserting if same file_path already processed.
    - If personal info (name, mobile) changed, update only those fields.
    """
    ...
    try:
        #  Step 1: Skip if file already processed
        cursor.execute(f"SELECT 1 FROM {table_name} WHERE file_path = %s LIMIT 1", (source_file,))
        if cursor.fetchone():
            logger.warning(f" File {source_file} already processed earlier. Skipping insertion.")
            cursor.close()
            conn.close()
            return 0

        # Step 2: Find existing unique records
        ...
        if insert_values:
            ...
            INSERT INTO {table_name} (
                brok_dlr_c, product, asset_date, folio, inv_name, mobile, scheme_nam, closing_as, city,
            ...
            for i in range(0, len(insert_values), BATCH_SIZE):
                ...
                try:
                    ...
                    logger.warning(f"Insert batch failed, retrying individually: {batch_error}")
                    conn.rollback()
                    for row in batch:
                        try:
        ...
        # Step 4: Update personal info only (inv_name, mobile)
        if update_values:
            update_query = f"""
            UPDATE {table_name}
            SET inv_name = %s,
